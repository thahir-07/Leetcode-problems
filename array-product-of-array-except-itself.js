...
function productExceptSelf(nums) {
    ...
    var postfix = new Array(nums.length).fill(1);
    var prefix = new Array(nums.length).fill(1);
    for (var i = 1; i < nums.length; i++) { //[1,2,3,4] prefix [1,1,2,6] (->) default 0 index 1
        prefix[i] = prefix[i - 1] * nums[i - 1];
    }
    for (var j = nums.length - 2; j > -1; j--) { //[1,2,3,4] postfix [24,12,4,1] (<-) default last index 1
...
function productExceptSelfImproved(nums) {
